...
function deleteCustomer(id) {
    ...
    if (!isCustomerListUnlocked) {
        showToast('請先輸入密碼以解鎖客戶清單', true);
        showCustomerListPassword();
        return;
    }
    ...
    const confirmMessage = `請再次確認刪除以下客戶資料：\n\n` +
        ...
        `月份：${{
            ...
            '2026-10': '2026年10月',
            '2026-11': '2026年11月'
        }[customer.orderMonth] || customer.orderMonth}\n` +
...
function updateMonthlyDisplay() {
    ...
    const extendedMonths = [
        { key: '2026-03', id: 'march' },
        { key: '2026-04', id: 'april' },
        { key: '2026-05', id: 'may' },
        { key: '2026-06', id: 'june' },
        { key: '2026-07', id: 'july' },
        { key: '2026-08', id: 'august' },
        { key: '2026-09', id: 'september' },
        { key: '2026-10', id: 'october' },
        { key: '2026-11', id: 'november' }
    ];

    extendedMonths.forEach(({ key, id }) => {
        if (monthlyStats[key]) {
            document.getElementById(`${id}-total`).textContent = `${monthlyStats[key].total.toLocaleString()}萬`;
            document.getElementById(`${id}-finance`).textContent = `${monthlyStats[key].finance.toLocaleString()}萬`;
            document.getElementById(`${id}-insurance`).textContent = `${monthlyStats[key].insurance.toLocaleString()}萬`;
        }
    });
}
    ...
    if (!isCustomerListUnlocked) {
        customerList.innerHTML = `
            <div style="text-align: center; padding: 40px;">
                <i class="fas fa-lock" style="font-size: 48px; color: #95a5a6; margin-bottom: 20px;"></i>
                <p style="color: #7f8c8d; font-size: 16px; margin-bottom: 20px;">客戶清單需要密碼驗證</p>
                <button onclick="showCustomerListPassword()" style="background: #3498db; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; font-size: 14px;">
                    <i class="fas fa-key"></i> 輸入密碼
                </button>
            </div>
        `;
        return;
...
function initializePage() {
    ...
    // 客戶清單密碼驗證表單事件
    document.getElementById('customerListPasswordForm').addEventListener('submit', function (e) {
        e.preventDefault();
        verifyCustomerListPassword();
    });
    ...
    window.addEventListener('click', function (e) {
        ...
        const customerListPasswordModal = document.getElementById('customerListPasswordModal');
        ...
        if (e.target === customerListPasswordModal) {
            closeCustomerListPassword();
        }
    });
...
// 客戶清單密碼功能
function showCustomerListPassword() {
    document.getElementById('customerListPasswordModal').style.display = 'block';
    document.getElementById('customerListPassword').focus();
}

function closeCustomerListPassword() {
    document.getElementById('customerListPasswordModal').style.display = 'none';
    document.getElementById('customerListPasswordForm').reset();
}

function verifyCustomerListPassword() {
...
document.addEventListener('DOMContentLoaded', function() {
    // Use event delegation since batchAction might not exist yet
...
async function executeBatchAction() {
    ...
    try {
        ...
        // Reload customer list after a short delay for Firestore to sync
        setTimeout(() => {
            loadBatchCustomers();
        }, 1000);
...
window.showCustomerListPassword = showCustomerListPassword;
window.closeCustomerListPassword = closeCustomerListPassword;
